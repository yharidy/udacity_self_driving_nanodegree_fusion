    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            for obj in detections:
                ...
                obj_corners = tools.compute_box_corners(obj[1], obj[2], obj[5], obj[6], obj[7])
                ## step 4 : compute the center distance between label and detection bounding-box in x, y, and z
                obj_x = obj[1].item() if isinstance(obj[1], torch.Tensor) else obj[1]
                obj_y = obj[2].item() if isinstance(obj[2], torch.Tensor) else obj[2]
                obj_z = obj[3].item() if isinstance(obj[3], torch.Tensor) else obj[3]
                dist_x = label.box.center_x - obj_x
                dist_y = label.box.center_y - obj_y
                dist_z = label.box.center_z - obj_z

                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                label_polygon = Polygon(label_corners)
                obj_polygon = Polygon(obj_corners)
                intersection = label_polygon.intersection(obj_polygon).area
                union = label_polygon.union(obj_polygon).area
